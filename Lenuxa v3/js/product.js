// ============================================================
// LUNEXA — product.js  (detail page with reviews + tracking)
// ============================================================
...
const FAKE_REVIEWS = [
  { name:'James M.',   rating:5, date:'Mar 2025', text:'Absolutely incredible quality. Exceeded all my expectations — fast shipping too.', verified:true },
  { name:'Sarah K.',   rating:5, date:'Feb 2025', text:'Perfect product. Exactly as described. The build quality is top-notch.', verified:true },
  { name:'Daniel W.',  rating:4, date:'Feb 2025', text:'Great value for money. Minor setup steps but overall a fantastic purchase.', verified:true },
  { name:'Priya N.',   rating:5, date:'Jan 2025', text:'Bought as a gift and the recipient absolutely loves it. Will buy again!', verified:true },
  { name:'Alex R.',    rating:4, date:'Jan 2025', text:'Solid performance and sleek design. Does exactly what I needed.', verified:false },
];
...
function renderProduct(product) {
  ...
  const buyers     = (product.reviews * Math.random() * 0.8 + 800).toFixed(0);
  const ratingDist = [72, 15, 7, 4, 2]; // % for 5,4,3,2,1 stars

  container.innerHTML = `
    <div class="product-gallery">
      <div class="product-main-img" id="main-img-wrap">
        <img id="main-product-img" src="${product.image}" alt="${product.name}"
             onerror="this.src='https://picsum.photos/seed/fb${product.id}/600/600'"/>
        ${discount > 0 ? `<span class="detail-badge-discount">-${discount}% OFF</span>` : ''}
      </div>
      <div class="product-thumbs">
        ${[0,1,2,3].map((i,idx) => {
          const seeds = ['a','b','c','d'];
          return `<div class="thumb ${idx===0?'active':''}" onclick="switchImage(this,'https://picsum.photos/seed/${product.id}${seeds[i]}/600/600')">
            <img src="https://picsum.photos/seed/${product.id}${seeds[i]}/120/120" alt="View ${i+1}"/>
          </div>`;
        }).join('')}
      </div>
    </div>
    ...
    <div class="product-info">
      <div class="product-info-top">
        ...
        <button class="detail-wishlist-btn ${isWished?'active':''}" data-wishlist-id="${product.id}"
      ...
      <div class="product-specs">
        ...
        <div class="specs-grid">
          ${product.specs.map(s=>`<div class="spec-item">
      ...
      <div class="product-highlights">
        ...
        <ul class="highlight-list">
          ...
          <li>Backed by manufacturer warranty and Lunexa support</li>
        </ul>
      </div>
      ...
      <div class="product-actions">
        <button class="btn-add-cart ${isInCart?'in-cart':''}" id="detail-cart-btn"
        ...
        <a href="checkout.html" class="btn-buy-now" onclick="if(!Cart.has(${product.id})){const q=parseInt(document.getElementById('product-qty').value)||1;Cart.add(${product.id},q);}">
          Buy Now
        </a>
      </div>
      ...
      <div class="trust-badges">
        ...
        <div class="trust-badge"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg><span>Free Shipping</span></div>
        <div class="trust-badge"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.51"/></svg><span>30-Day Returns</span></div>
      </div>
    </div>

    <!-- Rating summary + reviews (full width below) -->
        ...
        <div class="reviews-bars">
          ${ratingDist.map((pct,i) => `
            <div class="review-bar-row">
              <span class="review-bar-label">${5-i} stars</span>
        ...
        ${FAKE_REVIEWS.map(r => `
          <div class="review-card">
            <div class="review-header">
              ...
              <div>
                <div class="review-name">${r.name} ${r.verified?'<span class="review-verified">Verified Purchase</span>':''}</div>
                <div class="review-meta"><span class="review-stars">${'★'.repeat(r.rating)}</span> · ${r.date}</div>
              </div>
            </div>
...
function changeQty(delta) {
  ...
  if (input) input.value = Math.max(1, Math.min(99, parseInt(input.value)+delta));
}
...
function handleDetailAddToCart(productId) {
  ...
  if (btn) {
    ...
    btn.innerHTML = `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg> Go to Cart`;
...
function renderNotFound() {
  ...
  if (c) c.innerHTML = `<div class="not-found-state"><h2>Product Not Found</h2><p>This product does not exist or may have been removed.</p><a href="products.html" class="btn btn-primary btn-pill" style="margin-top:16px">Browse All Products</a></div>`;
}
