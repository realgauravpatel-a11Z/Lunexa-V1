// LUNEXA v4 — storage.js
const KEYS={USERS:'lunexa_users',CURRENT_USER:'lunexa_current_user',CART:u=>`lunexa_cart_${u||'guest'}`,WISHLIST:u=>`lunexa_wishlist_${u||'guest'}`,ORDERS:u=>`lunexa_orders_${u||'guest'}`,PRODUCTS:'lunexa_products_override',COOKIE_CONSENT:'lunexa_cookie_consent',ADMIN_AUTH:'lunexa_admin_session'};
const Store={
...
const Auth={
  encodePassword(p){return btoa(p+'_lunexa_v4_2025');},
  ...
  signup(name,email,password){
    ...
    Store.addUser(user);return{success:true,user};
  },
...
function showAuthGate(action='continue'){
  ...
  if(!overlay){
    ...
    overlay.innerHTML=`<div class="auth-gate-box"><button class="auth-gate-close" onclick="closeAuthGate()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button><div class="auth-gate-icon"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><h3 class="auth-gate-title">Sign in to continue</h3><p class="auth-gate-sub" id="auth-gate-msg">You need to sign in to ${action}.</p><div class="auth-gate-actions"><a href="login.html" class="auth-gate-btn-primary">Sign In</a><a href="signup.html" class="auth-gate-btn-secondary">Create Free Account</a></div></div>`;
    overlay.addEventListener('click',e=>{if(e.target===overlay)closeAuthGate();});
    document.body.appendChild(overlay);
  }
  const m=document.getElementById('auth-gate-msg');
  if(m)m.textContent=`You need to sign in to ${action}. It only takes a few seconds.`;
...
function redirectIfAuth(to='profile.html'){if(Store.isLoggedIn()){window.location.href=to;return true;}return false;}
